...
/**
 * [T-workbuddy-keepalive] AlarmManager entry point for the WorkBuddy
 * authorization keep-alive. Two triggers:
 *
 *  - ACTION_TICK: the periodic alarm fired. Runs one keep-alive pass
 *    (goes through [WorkBuddyKeepAlive.runKeepAliveNow], which refreshes
 ...
 */
class WorkBuddyKeepAliveReceiver : BroadcastReceiver() {

    companion object {
        private const val TAG = "WorkBuddyKeepAlive"
        const val ACTION_TICK = "com.openminis.app.action.WORKBUDDY_KEEPALIVE_TICK"
    }
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (intent.action) {
            ACTION_TICK -> {
                if (!WorkBuddyKeepAlive.isEnabled(appContext)) {
                ...
                CoroutineScope(SupervisorJob() + Dispatchers.IO).launch {
                    try {
                        ...
                        try {
                            WorkBuddyKeepAlive.scheduleNext(appContext)
                        } catch (e: Exception) {
            ...
            Intent.ACTION_BOOT_COMPLETED -> {
                ...
                WorkBuddyKeepAlive.rearmIfEnabled(appContext)
            }
        }
    }
}
