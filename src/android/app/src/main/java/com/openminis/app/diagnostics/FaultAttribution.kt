...
/**
 * [feat5-fault-attribution] Translate a provider failure's raw error text
 * into a LAYER attribution: connection-level (this device ↔ endpoint, the
 * user's network/proxy's fault) vs service-level (the endpoint answered,
 * so the relay/provider's fault) vs auth/quota (user must fix credentials).
 *
 ...
 * the Provider Health panel says "recent failure", the single most useful
 * next datum is WHOSE fault it likely was — that's what decides whether the
 * user checks their VPN or complains to their relay operator.
 *
